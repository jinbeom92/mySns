...
def sign_up_view(request):
    if request.method == 'GET':
        ...
        if password != password2:
            ...
            if exist_user:
                return render(request, 'user/signup.html',{'error':'사용자가 존재 합니다!'})
            else:
...
def sign_in_view(request):
    if request.method == 'POST':
        username = request.POST.get('username', '') #사용자가 작성한 요청을 username이라는 곳에서 들고와라
        password = request.POST.get('password', '')

        me = auth.authenticate(request, username=username, password=password)
        if me is not None: #비어있지 않다
            auth.login(request, me) #내 정보를 넣음
            return redirect('/')
        else:
